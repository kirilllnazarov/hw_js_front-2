...
function Clock() {
	...
	const stringTime = date.toLocaleTimeString("ru", { hour12: false }) || <br />; // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
	...
	const stringDay = new Intl.DateTimeFormat("en", { weekday: "long" }).format(date)[0].toUpperCase() +
		new Intl.DateTimeFormat("en", { weekday: "long" }).format(date).slice(1) || <br />; // пишут студенты
	const stringMonth = new Intl.DateTimeFormat("en", { month: "long" }).format(date)[0].toUpperCase() +
		new Intl.DateTimeFormat("en", { month: "long" }).format(date).slice(1) || <br />; // пишут студенты
